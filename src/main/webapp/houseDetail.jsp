    ...
    <div class="row">
      <div class="form_container">
        <form action="owner-servlet" method="post">
          ...
          <div style="padding: 12px 12px; margin-top: 25px">
            <p style="margin-bottom: 0">Giá</p>
          </div>
          <div style="padding: 12px 12px; margin-top: 25px">
            <p style="margin-bottom: 0">Diện tích</p>
          </div>
          <div style="padding: 12px 12px; margin-top: 25px">
            <p style="margin-bottom: 0">Loại</p>
          </div>
          <div style="padding: 12px 12px; margin-top: 25px">
            <p style="margin-bottom: 0">Tỉnh</p>
          </div>
          <div style="padding: 12px 12px; margin-top: 25px">
            <p style="margin-bottom: 0">Huyện</p>
          </div>
          <div style="padding: 12px 12px; margin-top: 25px">
            <p style="margin-bottom: 0">Phường, xã</p>
          </div>
          <div style="padding: 12px 12px; margin-top: 25px">
            <p style="margin-bottom: 0">Mô tả</p>
          </div>
        </form>
      </div>
      <div class="col-lg-4 col-md-5 ">
        <div class="form_container">
          <form action="owner-servlet" method="post">
            ...
            <div>
              <input type="number" name="price" placeholder="Giá" required="required" disabled="disabled" value="${house.getPrice()}"/>
            </div>
            <div>
              <input type="text" name="area" placeholder="Diện tich" required="required" disabled="disabled" value="${house.getArea()}" />
            </div>
            <div>
              <input type="text" name="type" placeholder="Loại (căn hộ, nhà đất...)" required="required" disabled="disabled" value="${house.getType()}" />
            </div>
            <div>
              <input type="text" name="province" placeholder="Địa chỉ (Tỉnh)" required="required" disabled="disabled" value="${house.getAddress().getProvince()}" />
            </div>
            <div>
              <input type="text" name="district" placeholder="Địa chỉ (Huyện)" required="required" disabled="disabled" value="${house.getAddress().getDistrict()}" />
            </div>
            <div>
              <input type="text" name="ward" placeholder="Địa chỉ (Phường, xã)" required="required" disabled="disabled" value="${house.getAddress().getWard()}" />
            </div>
            <div>
              <input type="text" class="message-box" name="description" placeholder="Mô tả" disabled="disabled" value="${house.getDescription()}" />
            </div>
            <div class="d-flex ">
              <button>
                Quay Lại
